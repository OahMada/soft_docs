...
一共有四条规则用于判断代码执行时的 `this` 所指, 我们需要检查代码找到函数的调用点, 然后判断哪条规则试用. 如果同时有不止一条规则可以应用于某个调用点, 那么需要根据 4 条规则的优先顺序判断究竟哪一条适用.
...
如果 `strict mode` 有生效, 那么*默认绑定*的 `this` 无法指向全局对象, 所以 `this` 的值被设为 `undefined`.
...
尽管 `bar` 看起来是 `obj.foo` 的一个引用, 实际上, 它只不过是对 foo 函数的另一个直接引用. `foo` 函数的调用点, 也就是 `bar()`, 本身是一个简单的, 无修饰的调用, 所以*默认绑定*规则应用于此.
...
JavaScript 中的函数大都会有的 `call()`, `apply()` 程序可以用于我们的目的, 来达到*直接绑定*的效果.
...
调用 `bind(..)` 函数后会返回一个新的函数, 这个新函数的原始函数的 `this` 绑定设置为了参数中指定的对象.
